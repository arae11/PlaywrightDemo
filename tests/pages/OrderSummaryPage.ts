...
export class OrderSummaryPage extends BasePage {
  ...
  async getOrderTotalText(locator: string): Promise<number> {
    ...
    if (isNaN(price))
      throw new Error(`Failed to parse order total price from "${text}"`);
    return price;
  }

  async verifyCorrectPriceOnSummaryPage(
    params: VerifyPriceParams
  ): Promise<number> {
    const {
      orderTotalLocator,
      railcard,
    ...
    const basePrice = this.promocodeHelper.getRailcardValueWithoutPromo(
      railcard,
      years
    );
    ...
    if (promo.trim() !== "") {
      // Step 3: Promo logic
      const { skipPayment } = await this.promocodeHelper.verifyPromocodeTags(
        promo,
        sku,
        basePrice
      );
      ...
      finalPrice = this.getFinalPriceConsideringSkipPayment(
        basePrice,
        skipPayment
      );

      // Step 5: Validate promocode via API (optional - logs response)
      const promoResponse = await this.promocodeHelper.validatePromocode(
        promo,
        sku,
        finalPrice
      );
    ...
    console.log(
      `✅ Expected Final Price: £${finalPrice.toFixed(
        2
      )} should match Actual Price: £${actualPrice.toFixed(2)}`
    );

    // Step 8: Assert prices match (throw error if not)
    if (Math.abs(finalPrice - actualPrice) > 0.01) {
      throw new Error(
        `Price mismatch! Expected £${finalPrice.toFixed(
          2
        )}, but got £${actualPrice.toFixed(2)}`
      );
    }
